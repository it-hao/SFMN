...
class TestData(data.Dataset):
    def __init__(self, dataset_name, val_data_dir, downsample):
        ...
        with open(val_list) as f:
            ...
            if self.dataset_name=='nh' or self.dataset_name=='dense' or self.dataset_name=='ohaze':
    ...
    def get_images(self, index):
        ...
        # haze_img = cv2.cvtColor(cv2.imread(os.path.join(self.val_data_dir, 'hazy', haze_name)), cv2.COLOR_BGR2RGB)
        haze_img  = imageio.imread(os.path.join(self.val_data_dir, 'hazy', haze_name))
        
        gt_name = self.gt_names[index]
        # gt_img = cv2.cvtColor(cv2.imread(os.path.join(self.val_data_dir, 'gt', gt_name)), cv2.COLOR_BGR2RGB)
